...
'5.2': 'NORMETICv1.2_element_5_2_type_de_ressource_voc.xml',
...
for (fieldnum, filename) in files.items():
    xml = parse(urlopen(eureka+filename))
    for lang in ('en', 'fr'):
        with open('web/vocab/%s/LOM_%s.html'%(lang, fieldnum),'w') as html:
            xsl(xml, lang="'%s'"%(lang,), fieldnum="'%s'"%(fieldnum,)).write(html, encoding='utf-8', method='html', pretty_print=True)
